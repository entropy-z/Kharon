...
auto DECLFN Process::Create(
    ...
    Success = Kh->Krnl32.CreateProcessA( 
        ...
        0, Kh->PsCtx.CurrentDir, &SiEx.StartupInfo, PsInfo
    );
    if ( !Success ) goto _KH_END;

    KhDbg( "process create at pid: %d tid: %d", PsInfo->dwProcessId, PsInfo->dwThreadId );

    if ( Kh->PsCtx.Pipe ) {
        // Kh->Krnl32.WaitForSingleObject( PsInfo->hProcess, 1000 );
        Kh->Ntdll.NtClose( PipeWrite );
    
        Success = Kh->Krnl32.PeekNamedPipe( PipeRead, 0, 0, 0, &PipeBuffSize, 0 );
        if ( !Success ) goto _KH_END;

        PipeBuff = (PBYTE)Kh->Hp->Alloc( PipeBuffSize );
        if ( !PipeBuff ) goto _KH_END;

        KhDbg( "pipe buffer at %p", PipeBuff );
    
        Success = Kh->Krnl32.ReadFile( PipeRead, PipeBuff, PipeBuffSize, &TmpValue, 0 );
        if ( !Success ) goto _KH_END;
    
        if ( Package ) Kh->Pkg->AddBytes( Package, PipeBuff, PipeBuffSize );    
    }
