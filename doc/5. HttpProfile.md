...
| `format` | string | Encoding format: `base64` (others planned: `base32`, `base64url`) |
...
| `format` | string | Encoding format: `base64` (others planned: `base32`, `base64url`, `hex`) |
| `header` | string | (Optional) Send output in HTTP header with this name |
...
XOR Encryption (symmetric key cipher)
    ↓
Format Encoding (base64/base32/base64url)
    ↓
